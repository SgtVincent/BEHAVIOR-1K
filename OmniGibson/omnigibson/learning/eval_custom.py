...
class Evaluator:
    ...
    def load_env(self, env_wrapper: DictConfig) -> EnvironmentWrapper:
        ...
        # Keep the canonical task scene at env bootstrap time. Raw / merged scene snapshots can omit
        # pedestrians and presampled robot pose metadata, which leaves the viewer showing an empty scene
        # with the robot spawned at the origin before task-instance reset has a chance to run.
